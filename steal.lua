...
lib.makelabel("This game has VERY good anticheat! Howeveryou can just click on he base you wanna go to (toggle select base) and then click goto base!",main)
...
game.ReplicatedStorage.Packages.Net["RE/StealService/DeliverySteal"]:FireServer()
                       elseif
game.ReplicatedStorage.Packages.Net:FindFirstChild("RE/3891348e-5b69-47f3-af95-20012defb3fe") then
                            game.ReplicatedStorage.Packages.Net["RE/3891348e-5b69-47f3-af95-20012defb3fe"]:FireServer("e280cd99-2836-4a9c-8b9e-59e5750aab98")
                       else
                            print("steal not found!")
                       end
...
local gtb = false
lib.maketoggle("Goto Base (waits for the timer to end and walks in)",main,function(bool)
     gtb = bool
...
local notusing = true
lib.makebutton("Goto Base (WILL OOF YOU, ONLY WORKS IN OLD SERVERS)",main,function()
...
lib.makebutton("Goto Base (Second Floor, WILL OOF YOU, ONLY WORKS IN OLD SERVERS)",main,function()
     if gotobase and notusing then
         notusing = false
         pcall(function()
...
table.insert(tableofconnections,p.Base.Spawn.PromptAttachment.ChildAdded:Connect(
function(c)
...
or c:IsA("HingeConstraint")) and c and c.Parent then
      ...
      localplr.Character.Humanoid:ChangeState(Enum.HumanoidStateType.GettingUp)
                        localplr.Character.Humanoid.PlatformStand = false --finally
